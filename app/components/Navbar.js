...
const Navbar = () => {
  ...
  return (
    <nav className={`w-full fixed top-0 z-50 transition-all duration-300 ${scrolling ? 'bg-blue-900/90 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-8 lg:px-16 py-4">
        ...
        <div className="flex justify-between items-center gap-8">
          ...
          <FiAlignJustify 
            ...
            className={`md:hidden text-3xl cursor-pointer text-white transition-transform ${isOpn ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'}`}
          />
        </div>
